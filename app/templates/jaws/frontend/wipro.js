...
$(document).ready(function() {
	...
	$("a#btn-wipro").click(function() {
		...
		// LoB Approval
		try {
			if ($('input#ichk-approval').is(":checked")) approval = "Yes";
		} catch(err) {}
		...
		if ((email.length < 9) || ((email.indexOf('@wipro.com') == -1))) {
			$("label.error[for='txt-email']").html("Enter your valid Wipro email ID.");
		...
		if (empid.length < 1) {
			$("label.error[for='txt-emp-id']").html("Enter your Wipro Employee ID.");
		...
		if (office.length < 1) {
			$("label.error[for='txt-office']").html("Enter your Wipro office location.");
		...
		window.location.href= $("#txt-jaws-url").val() + "/wipro/pay?" +
			...
			"approval=" + encodeURIComponent(approval) + "&" +
