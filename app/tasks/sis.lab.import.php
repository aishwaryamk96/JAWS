...
*/
    ...
    if ($old_sis_batch_id && strcmp($old_sis_batch_id, "nil") != 0)
    {
        ...
        if (strcmp($result["workflow_state"], "imported") != 0)
        {
            ...
            activity_create("critical", "sis.import.automation", "fail", "", "", "", "", "Import of sis-".$filename_timestamp.".zip failed on LMS", "pending");
            ...
            if (isset($result["processing_warnings"]) || isset($result["processing_errors"]))
            {
                if (isset($result["processing_warnings"]))
                    $message .= " warning: <br /><br />".$result["processing_warnings"]."<br /><br />";
                if (isset($result["processing_errors"]))
                    $message .= " and following error: <br /><br />".$result["processing_errors"]."<br /><br />";
            }
            else if (isset($result["data"]["error_message"]))
                $message .= " error message: <br /><br />".$result["data"]["error_message"]."<br /><br />";

            $message .= "Please take a look at the issue.<br />Please <a href='https://jigsawacademy.com/jaws/view/backend/temp/sis.get'>click here</a> to download the troublesome file.<br /><br />
            Thank you,<br />
            JAWS
            </body>
            </html>";
    ...
    $res_sis = db_query("SELECT enr_id, user_id, subs_id, course_id, learn_mode, sis_id, lms_pass, section_id FROM user_enrollment WHERE sis_status='na' AND (il_start_date IS NULL OR il_start_date=CURDATE())");
    ...
    if ($res_sis)
    {
        ...
        foreach ($res_sis as $sis_record)
        {
        	...
        	if (($prev_user_id != $sis_record["user_id"]) && $new_user)
        	{
    	    	...
    	    	$user_lines .= $sis_record["sis_id"].",".$res_user["soc_".$res_user["lms_soc"]].",".$soc.",".$sis_record["lms_pass"].",".$res_user["name"].",,".$res_user["name"].",".$res_user["email"].",";
            ...
            db_exec("UPDATE user_enrollment SET sis_file=".db_sanitize($filename)." WHERE enr_id=".$sis_record["enr_id"]);
        }
        ...
        while (!$result)
        {
        	...
        	else
        	{
        		...
        		else if (strcmp($result["workflow_state"], "failed") == 0)
        		{
                    ...
                    activity_create("critical", "sis.import.automation", "fail", "", "", "", "", "Import of sis-".$filename_timestamp.".zip returned error");
                    break;
        		}
