...
*/
	...
	if ($_POST["criterion"] == "email")
		...
		$user = db_query("SELECT * FROM user WHERE phone=".$_POST["text"]);
	...
	/*$res_lead = db_query("SELECT * FROM user_leads_basic WHERE email=".db_sanitize($_POST["email"]));
	$res_lead2 = db_query("SELECT * FROM user_leads_basic_compiled WHERE email=".db_sanitize($_POST["email"]));

	if (isset($res_lead[0]) || isset($res_lead2[0]))
	{
		echo "Referral has already contacted us.";
		exit();
	}*/
