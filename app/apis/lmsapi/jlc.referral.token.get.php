...
*/
	...
	if (!$user)
	{
		...
		$res_act = db_query("SELECT * FROM system_activity WHERE act_type='jlc.user.not_found' AND activity=".db_sanitize($_POST["email"]));
		die(json_encode(array("token" => psk_generate("system_activity", $res_act[0]["act_id"], "jlc.referral.get"))));
	}

	die(json_encode(array("token" => psk_generate("user", $user["user_id"], "jlc.referral.get"))));
