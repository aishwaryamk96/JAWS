...
*/
	...
	if (!isset($_POST["name"]) || !isset($_POST["email"]) || !isset($_POST["phone"]) || !isset($_POST["city"])) {
		activity_create("critical","epba.paylink.create","fail","","","","","Missing Basic Field in API.");
		die(json_encode(['error'=>'Missing Basic Fields']));
	}

	// Extra Check
	if ( !isset($_POST["file_work_cv"]) ) {
		activity_create("critical","epba.paylink.create","fail","","","","","Missing Extra Field in API.");
	...
	$act_id = activity_create('high','epba.paylink','use','','','','',json_encode($data),'pending');
	echo json_encode(["link" => 'https://www.jigsawacademy.com/jaws/epba/pay?pay='.$act_id]);
