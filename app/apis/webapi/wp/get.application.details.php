...
//    $format = $course_details[0]['app_num_format'];
    $format = getCourseApplicationFormat($course_id);
    if( empty($format) ){
        echo json_encode( array('status'=> false) );
        exit;
    }
    
    $payId = db_query( "SELECT `pay_id` FROM `payment_link`  WHERE web_id=". db_sanitize($web_id) );
    $payId = $payId[0]['pay_id'];
...
//    $application_number = $format . date('m') . date("d") . $last_app_num;
    $application_number = getUniqueApplicationId($payId, $format, true);
