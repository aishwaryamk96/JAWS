...
*/
	...
	$transaction_info['sum'] = 4025; //($_GET['course_code'] == 'fsas' ? 28750 : 42000);
	$transaction_info['currency'] = strtoupper($_GET['paymode']);
	$transaction_info['extra']['desc'] = 'DSB_TBLHDPSPK_INR_2017';
	...
	// Store in Activity
	$act_id = activity_create('high','dsb.paylink','use','','','','',json_encode($data),'pending');

	// Send Email Here ???

	// Prep more
	$transaction_info['invoice_id'] = $act_id;
	$transaction_info['return_url'] = JAWS_PATH_WEB."/dsb/pay/success?pay=".$act_id;
	...
	// Transact
	payment_transact($transaction_info, "dsb.paylink", $act_id, "dsb.paylink.confirm");
