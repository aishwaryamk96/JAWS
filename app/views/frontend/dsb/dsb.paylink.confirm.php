...
*/
	...
	// Parse Activity
	$act = db_query('SELECT content FROM system_activity WHERE act_type="dsb.paylink" AND activity="use" AND status="pending" AND act_id='.intval($_GET["pay"]).' LIMIT 1;');

	// Check Activity
	if (!isset($act[0])) {
		activity_create("low", "dsb.paylink.confirm", "fail", "act_id", $_GET["pay"], "", '', "Activity Record Not Found", "logged");
		ui_render_msg_front(array(
			"type" => "error",
			"title" => "A problem ran into you :(",
			"header" => "Oops !",
	...
	if ($transaction_response === false) {
		activity_create("low", "dsb.paylink.confirm", "fail", "act_id", $_GET["pay"], "", '', "No Transaction Response", "logged");
		ui_render_msg_front(array(
			"type" => "error",
			"title" => "A problem ran into you :(",
			"header" => "Oops !",
	...
	// Validate Response
	if (!payment_validate("dsb.paylink", intval($_GET["pay"]), "dsb.paylink.confirm")) {
		activity_create("low", "dsb.paylink.confirm", "fail", "act_id", $_GET["pay"], "", "", "PSK Mismatch", "logged");
		ui_render_msg_front(array(
			"type" => "error",
			"title" => "A problem ran into you :(",
			"header" => "Oops !",
	...
	if (!$transaction_response["status"]) {
		activity_create("high", "dsb.paylink.confirm", "fail", "act_id", $_GET["pay"], "", '', "Gateway Cancelled/Failed", "logged");
		ui_render_msg_front(array(
			"type" => "error",
			"title" => "A problem ran into you :(",
			"header" => "Oops !",
