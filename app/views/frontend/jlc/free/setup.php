...
*/
	...
	// Prep
	$name; $email; $login_email; $phone; $password; $psk; $course=intval($_REQUEST['course']);

	// Get Course
	$course_info = course_get_info_by_id($course);
	...
	if (!isset($_REQUEST['corp'])) {
		...
		if (empty($_REQUEST["soc"])) {
			...
			if (!empty($_SESSION['user']["soc_gp"])) {
				$_REQUEST["soc"] = "gp";
			}
			else if (!empty($_SESSION['user']["soc_fb"])) {
				$_REQUEST["soc"] = "fb";
			}
			else if (!empty($_SESSION['user']["soc_li"])) {
				$_REQUEST["soc"] = "li";
			}

		}

		$login_email = $_SESSION["user"]["soc_".strtolower($_REQUEST["soc"])];

		//active campaign lead sync
		// ac_event_contact_iot($_SESSION['user'],$_REQUEST,'iot,iot-registration');
	...
	// Mode : Corp
		...
		//active campaign lead sync
		// ac_event_contact_iot($_POST,$_POST,'iot,iot-lp');
	}
	...
	if (isset($enr_old_act[0])) {
		...
		$_REQUEST[$content["lms_auth_key"]] = $content["lms_auth_value"];
	}

	// Create Tracking Activity
	$enract = db_query('SELECT * FROM system_activity WHERE act_type="jlc.free" AND activity="setup" AND content='.db_sanitize($email).' AND entity_type="course" AND entity_id='.$course.';');
	...
	if (isset($enract[0]['status']) && ($enract[0]['status'] == 'executed')) {
		...
		if (isset($enract[0]['status'])) $psk = psk_get('jlc.free', $enract[0]['act_id'], 'setup');

		// Not registered for this course
		else {
			$act_id = activity_create('high','jlc.free','setup','course',$course,'','',$email,'pending');
			...
			if (isset($enract[0]['status']) && ($enract[0]['status'] == 'executed')) {
				...
				if ($regged['lms_auth_key'] == 'soc') {
					$_REQUEST['soc'] = $regged['lms_auth_value'];
					unset($_REQUEST['corp']);
				}
				else {
					$_REQUEST['corp'] = true;
...
<?php
	...
	if (enrollment_free_create($login_email, $name, (!isset($_REQUEST["corp"])) ? "soc" : "pass", (!isset($_REQUEST["corp"])) ? $_REQUEST["soc"] : $password, $course)) {
