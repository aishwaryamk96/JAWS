...
*/
	...
	$act = db_query('SELECT content FROM system_activity WHERE act_type="uc.paylink" AND activity="use" AND status="pending" AND act_id='.$act_id.' LIMIT 1;');

	// Check Activity
	if (!isset($act[0])) {
		activity_create("low", "uc.paylink.parse", "fail", "act_id", $act_id, "", '', "Activity Record Not Found", "logged");
		ui_render_msg_front(array(
			"type" => "error",
			"title" => "A problem ran into you :(",
			"header" => "Oops !",
	...
	$transaction_info['sum'] = intval($data["sum"]);
	$transaction_info['currency'] = $data["paymode"];
	$transaction_info['extra']['desc'] = 'UC_UNI_INR_2017';

	// Prep more
	$transaction_info['invoice_id'] = $act_id;
	$transaction_info['return_url'] = JAWS_PATH_WEB."/uc/pay/success?pay=".$act_id;
	...
	// Transact
	payment_transact($transaction_info, "uc.paylink", $act_id, "uc.paylink.confirm");
