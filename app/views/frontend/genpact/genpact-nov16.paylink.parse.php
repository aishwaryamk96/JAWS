...
*/
	...
	$transaction_info['sum'] = ($_GET['course_code'] == 'fsas' ? 28750 : 42000);
	$transaction_info['currency'] = strtoupper($_GET['paymode']);
	$transaction_info['extra']['desc'] = ($_GET['course_code'] == 'fsas' ? 'GENPACT_FSAS_'.strtoupper($_GET['paymode']).'_2016' : 'GENPACT_ADSS_'.strtoupper($_GET['paymode']).'_2016');
	...
	// Store in Activity
	$act_id = activity_create('high','genpact.paylink','use','','','','',json_encode($data),'pending');

	// Send Email Here ???

	// Prep more
	$transaction_info['invoice_id'] = $act_id;
	$transaction_info['return_url'] = JAWS_PATH_WEB."/genpact/pay/success?pay=".$act_id;
	...
	// Transact
	payment_transact($transaction_info, "genpact.paylink", $act_id, "genpact.paylink.confirm");
