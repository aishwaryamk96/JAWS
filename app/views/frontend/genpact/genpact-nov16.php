...
*/
	...
	$course_price = '42,000 INR';

	if (strpos(strtolower($_SERVER['REQUEST_URI']), 'genpact-fsas') !== false) {
		$course_name = 'Foundation Course in Analytics';
		$course_price = '28,750 INR';
	}

	// Proceed with rendering the UI
	ui_render_head_front(array(
		"title" => $course_name." (GENPACT)",
		"scripts" => array(1 => "app/templates/jaws/frontend/genpact-nov16.js"),
...
?>
		...
		<div id="wns-form" class="border"> 
			...
			<div class="sub-header">GENPACT - <?php echo $course_name; ?></div>
			<div class="intro">
				Enroll for '<?php echo $course_name; ?>' with us and avail a discount if you are a GENPACT Employee.<br/>Fill the form below to enroll yourself.
			</div>     
		...
		<br/>
			...
			<div class="title"><span>Select your Mode of Payment</span></div>
			<div id="paymode" class="border">
					<br/>
					<form>   
						<input type="radio" name="opt-mode" value="inr" id="iopt-inr" checked="checked"/><label for="iopt-inr" selected><span><i class="fa fa-fw fa-lg fa-check"></i>   </span>Indian Rupees (<?php echo $course_price; ?> Incl. of all taxes)</label><br/>
			</form>
			</div>
		...
		<div style="visibility: hidden; display: none;">
			...
			<input type="hidden" value="<?php echo ((strpos(strtolower($_SERVER['REQUEST_URI']), 'genpact-fsas') !== false) ? 'fsas' : 'adss'); ?>" id="txt-course-code" style="visibility: hidden; display: none;" />
		</div>
