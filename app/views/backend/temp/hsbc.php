...
*/
    ...
    // Prep
    $login_params["return_url"] = JAWS_PATH_WEB."/hsbcform";
    ...
    // Priviledge Check
    if (!auth_session_is_allowed("hsbccorp.get")) {
	...
	try {
		phpexcel_write($lqueryres_arr, $prop, "HSBC (".date("F j, Y").").xls");
	}
	catch(Exception $e) {
		var_dump($e);
	}
	exit();
    	...
    	<html>
		<head>
			<title>JAWS - HSBC Sign-Up Report (Temp)</title>
		</head>
		<body>
			<center>
			<b>HSBC Report Download</b>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color: gray;">Logged in as <?php echo $_SESSION["user"]["name"]; ?>!</span>&nbsp;<a href="https://www.jigsawacademy.com/jaws/logout">Logout</a>
			<hr>
			<form method="POST" action="https://www.jigsawacademy.com/jaws/hsbcform" style="text-align: left; width: 50vw; margin: 0 auto;">
