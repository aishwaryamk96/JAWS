...
*/
	...
	// Prep
	$login_params["return_url"] = JAWS_PATH_WEB."/aform";
	...
	$data = package_fetch($start,$limit,$email);
	
	$total = package_fetch_count($email);
						...
						<div class="accordian-panel pin has-subpanel">
							...
							<div class="panel-main">
								<div class="content-min overflow" style="opacity: 1; pointer-events: auto;">
									...
									<?php if($package['creator_id'] > 0){ ?>
										<?php $agent = user_get_by_id($package['creator_id']); ?>
										<?php echo $agent['name']; ?>
									<?php } else { echo "Website"; } ?>
									</div>
									<div class="section-col">
...
<!-- Pagination -->
					<div class="section-boxed" style="text-align: center;">
						<?php echo pagination($total,$page,$limit,$filter); ?>
					</div>
<!-- Pagination -->
...
<style>.user_details{width:235px;}.user-courses{width:220px;}.user-status{width:100px;}.user-package{width:90px;}.agent-details{width:110px;}.details-section div.accordian-panel div.content-min div { display: inline-block; }.btn{height: 30px;padding: 0;min-width: 0;margin: 0 5px;box-shadow: unset; border: none; }.btn-warning{background:#FF0000;color:#FFFFFF;}.btn-submit{background:#90EE90;color:#FFFFFF;}</style>		
